...
def take_vote_from_database(player, day, players_db, mode='ST'):
    ...
    try:
        if mode == 'FG':
            return [data[2] for data in players_db[player] if
                    data[0] == day][0]
        else:
            return [data[3] for data in players_db[player] if
                    data[0] == day][0]
    except (KeyError, IndexError):
        return 'n.e.'


def players_with_vote(list_of_tuples, players_db, mode='ST'):
    ...
    for player in list_of_tuples:
        ...
        if mode == 'FG':
            vote = take_vote_from_database(player[1], day, players_db, 'FG')
        else:
            vote = take_vote_from_database(player[1], day, players_db)
...
def mantra_simulation(lineup, module, players_db, mode='ST'):
    ...
    if mode == 'FG':
        field, bench = players_with_vote(clean_lineup, players_db, 'FG')
    else:
        field, bench = players_with_vote(clean_lineup, players_db)
